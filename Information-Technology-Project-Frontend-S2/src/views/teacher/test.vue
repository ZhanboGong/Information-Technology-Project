  ...
  <div class="space-y-6 animate-fade-in max-w-[1400px] mx-auto pt-2 pb-20">
                ...
                <div v-for="(item, idx) in form.rubric_config.items" :key="idx" class="relative p-6 bg-white/50 border border-gray-200 rounded-3xl shadow-sm group">
                  ...
                  <div class="grid grid-cols-1 lg:grid-cols-5 gap-3 pt-6 border-t border-dashed border-gray-200">
                    <div v-for="level in gradeLevels" :key="level" class="space-y-2">
                      ...
                      <textarea v-model="item.detailed_rubric[level]" class="w-full bg-gray-50/50 border border-gray-100 rounded-xl p-3 text-[11px] h-36 focus:bg-white outline-none leading-relaxed"></textarea>
                    </div>
                  </div>
                </div>
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div v-for="assign in courseAssignments" :key="assign.id" 
            ...
            <div>
              ...
              <p class="text-sm text-gray-500 line-clamp-2 mb-2 leading-relaxed">{{ assign.content }}</p>
      ...
      <div v-if="selectedAssignment" class="space-y-6 p-2">
        ...
        <section v-if="matrixData.length > 0">
          <el-table :data="matrixData" border stripe class="rubric-matrix-table shadow-sm">
            <el-table-column label="Grade" width="200" fixed="left">
            ...
            <el-table-column v-for="col in matrixColumns" :key="col.prop" :min-width="280">
              <template #header>
                <div class="flex flex-col py-1">
                  <span class="text-gray-800 font-bold">{{ col.label }}</span>
                  <span class="text-[10px] text-blue-500 font-black">WEIGHT: {{ col.weight }}%</span>
                </div>
              </template>
              <template #default="scope">
    ...
    <div v-if="showImportModal" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/40 backdrop-blur-sm p-4">
        <div class="bg-white rounded-3xl w-full max-w-lg p-8 shadow-2xl relative">
            ...
            <h3 class="text-xl font-bold mb-6 flex items-center gap-3"><FileSpreadsheet class="text-emerald-600" size="28" /> Student Import</h3>
            <div class="flex justify-end gap-4 mt-8">
                <button @click="showImportModal = false" class="px-6 py-3 text-gray-500 font-bold">Cancel</button>
                <input type="file" ref="fileInput" @change="handleImportStudents" class="hidden" />
                <button @click="$refs.fileInput.click()" :disabled="isImporting" class="px-8 py-3 bg-emerald-500 text-white font-bold rounded-2xl shadow-lg shadow-emerald-100">
...
const jumpToStep = (step) => { if (step <= currentStep.value) currentStep.value = step; };
const handleNext = () => {
...
const resetToCourseList = () => { selectedCourse.value = null; isEditMode.value = false; };
...
const handleImportStudents = async (e) => {
  ...
  try {
    await api.post('/api/auth/teacher/students/import-students/', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
