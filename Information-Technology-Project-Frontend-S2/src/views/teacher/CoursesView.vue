  ...
  <div class="space-y-6 animate-fade-in max-w-[1400px] mx-auto pt-2 pb-20">
    ...
    <template v-if="!selectedCourse">
      <div class="flex justify-between items-end mb-6">
        ...
        <div class="flex items-center gap-3">
          <div class="flex items-center bg-gray-100 rounded-lg p-1">
            <button @click="viewMode = 'card'" :class="viewMode === 'card' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-400'" class="p-1.5 rounded-md transition-all" title="Card View">
              <LayoutGrid size="16" />
            </button>
            <button @click="viewMode = 'list'" :class="viewMode === 'list' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-400'" class="p-1.5 rounded-md transition-all" title="List View">
              <List size="16" />
            </button>
          </div>
          <button 
      ...
      <div v-if="isLoading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="i in 6" :key="i" class="frosted-card p-0 rounded-2xl overflow-hidden border border-white/60 animate-pulse">
      ...
      <div v-else-if="viewMode === 'card'" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div v-for="(course, index) in courses" :key="course.id" class="frosted-card p-6 rounded-2xl overflow-hidden group hover:shadow-xl hover:-translate-y-1 transition-all border-2 border-gray-100 hover:border-blue-500 hover:shadow-blue-100 flex flex-col h-full cursor-pointer" @click="openCourseManagement(course)">
    <div class="flex-1 flex flex-col">
        <div class="flex justify-between items-start mb-3">
            ...
            <div class="flex flex-col items-end gap-1">
                ...
                <div class="flex gap-1 mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button 
                        @click.stop="handleEditCourseInfo(course)" 
      ...
      <div v-else class="space-y-2">
        <div v-for="(course, index) in courses" :key="course.id"
             ...
             class="frosted-card p-4 rounded-xl border border-white/60 hover:shadow-md hover:border-blue-200 transition-all cursor-pointer flex items-center justify-between group">
          <div class="flex items-center gap-4">
            <div class="w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold text-sm" :class="cardColors[index % cardColors.length]">
              {{ course.name.charAt(0) }}
            </div>
            <div>
              <h3 class="font-bold text-gray-800 group-hover:text-blue-700 transition-colors">{{ course.name }}</h3>
              <p class="text-xs text-gray-400">ID:{{ course.id }} · {{ course.student_count }} students</p>
            </div>
          </div>
          <div class="flex items-center gap-6 text-sm text-gray-500">
            <span class="flex items-center gap-1"><Calendar size="14" class="text-gray-400"/> {{ formatDate(course.created_at) }}</span>
            <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button @click.stop="handleEditCourseInfo(course)" class="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg"><Edit size="14"/></button>
              <button @click.stop="confirmDeleteCourse(course)" class="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg"><Trash2 size="14"/></button>
            </div>
          </div>
        </div>
      </div>
      <div v-if="currentPage < totalPages" class="flex justify-center mt-8">
        <button @click="fetchCourses(currentPage + 1, true)" :disabled="isLoadingMore"
                class="px-8 py-3 bg-white border border-gray-200 text-gray-600 rounded-xl font-bold hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 transition-all shadow-sm flex items-center gap-2 disabled:opacity-50">
          <Loader2 v-if="isLoadingMore" size="16" class="animate-spin" />
    ...
    <template v-else>
      ...
      <div v-if="activeTab === 'publish_assignment'" class="animate-fade-in pb-20">
        <div class="frosted-card p-4 rounded-2xl border border-white/60 mb-8 flex justify-between items-center relative overflow-hidden">
          <div class="absolute top-1/2 left-10 right-10 h-1 bg-gray-200 -translate-y-1/2 z-0 rounded-full">
            <div class="h-full bg-gradient-to-r from-blue-500 to-indigo-600 transition-all duration-500" :style="{ width: ((currentStep - 1) / 3) * 100 + '%' }"></div>
          </div>
          <div v-for="step in 4" :key="step" class="relative z-10 flex flex-col items-center w-32 cursor-pointer" @click="jumpToStep(step)">
            ...
            <span class="text-xs font-bold mt-2" :class="currentStep > step ? 'text-emerald-600' : currentStep === step ? 'text-blue-800' : 'text-gray-400'">{{ stepTitles[step-1] }}</span>
          </div>
        </div>
          ...
          <div v-if="currentStep === 1" class="space-y-6">
            <div class="frosted-card p-8 rounded-2xl border border-white/60 space-y-5">
                 ...
                 <div>
                   ...
                   <input v-model="form.title" type="text" class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all">
                 </div>
                 <div>
                 ...
                 <div>
                   ...
                   <textarea v-model="form.content" class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm h-32 resize-none outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all"></textarea>
                 </div>
              ...
              <div class="flex justify-between items-center mb-6">
                <div>
                  ...
                  <button 
                    ...
                    class="flex items-center gap-2 px-4 py-1.5 bg-indigo-50 text-indigo-600 rounded-xl text-xs font-black border border-indigo-100 hover:bg-indigo-600 hover:text-white transition-all shadow-sm disabled:opacity-50"
                  >
                    <Sparkles v-if="!isGeneratingRubric" size="14" />
                    <Loader2 v-else size="14" class="animate-spin" />
                    AI One-Click Generate
                  </button>
          ...
          <div v-if="currentStep === 3" class="space-y-6">
            <div class="frosted-card p-8 rounded-2xl border border-white/60">
              <div class="space-y-6">
                ...
                <div v-if="selectedKPs.length > 0" class="mt-3 space-y-2">
                    <div v-for="kp in selectedKPs" :key="kp.id" 
                         class="flex items-center justify-between p-3 bg-blue-50 border border-blue-100 rounded-xl group hover:border-blue-300 transition-all">
                        <div class="min-w-0 flex-1">
                            <p class="text-sm font-bold text-gray-800 truncate">{{ kp.name }}</p>
                            <p class="text-[11px] text-gray-400 truncate mt-0.5">{{ kp.description }}</p>
                        </div>
                        <div class="flex items-center gap-1 ml-3 shrink-0 opacity-60 group-hover:opacity-100 transition-opacity">
                            <button @click="startEditKP(kp.id)" class="p-1.5 text-blue-400 hover:text-blue-700 hover:bg-blue-100 rounded-lg transition-all" title="Edit">
                                <Pencil size="14" />
                            </button>
                            <button @click="removeKP(kp.id)" class="p-1.5 text-red-400 hover:text-red-700 hover:bg-red-50 rounded-lg transition-all" title="Remove">
                                <X size="14" />
                            </button>
                        </div>
                    </div>
                    ...
                    <div v-for="(kp, i) in aiSuggestions" :key="i" class="bg-white p-4 rounded-xl border border-amber-100 shadow-sm hover:border-amber-400 transition-all">
                      <div class="flex justify-between items-start mb-2">
                        <div class="flex items-center gap-2">
                          ...
                          <span v-if="kp.bloom_level" class="px-1.5 py-0.5 bg-indigo-50 text-indigo-600 rounded text-[9px] font-black uppercase">
                            {{ kp.bloom_level }}
                          </span>
                        </div>
                ...
                <div class="p-6 bg-indigo-50 rounded-2xl border border-indigo-100 flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p class="text-sm text-indigo-700 mt-1 leading-relaxed">The selected knowledge points will be used as the core reference dimension for AI marking.</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
              ...
              <tbody class="text-sm divide-y divide-gray-100/80">
                <tr v-for="stu in filteredStudents" :key="stu.id" class="hover:bg-blue-50/60 group transition-colors">
      ...
      <div v-if="activeTab === 'assignments_list'" class="animate-fade-in space-y-4 pb-20">
        ...
        <div v-else-if="courseAssignments.length === 0" class="p-10 text-center text-gray-400 bg-white/50 rounded-2xl border border-dashed border-gray-200 flex flex-col items-center">
            ...
            <p class="font-bold text-gray-500 mb-1">No assignments published yet</p>
            <p class="text-xs text-gray-400 mb-6">Create your first assignment to get started</p>
            <button @click="enterCreateMode" class="px-6 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-bold shadow-lg hover:bg-blue-700 transition-all flex items-center gap-2">
              <FilePlus size="16" /> Publish First Assignment
            </button>
        </div>
...
<div v-for="assign in courseAssignments" :key="assign.id" 
  ...
  <div>
    <div class="flex justify-between items-start mb-2">
      ...
      <div class="flex gap-2">
        <button @click="handleEditAssignment(assign)" class="p-1.5 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-600 hover:text-white transition-all shadow-sm" title="Edit">
          <Edit size="14" />
        </button>
        <button @click.stop="handleDeleteAssignment(assign)" class="p-1.5 bg-red-50 text-red-600 rounded-lg hover:bg-red-600 hover:text-white transition-all shadow-sm" title="Delete">
          <Trash2 size="14" />
        </button>
  ...
  <div class="flex items-center gap-4 text-xs text-gray-400 mt-2 mb-2">
    <span class="flex items-center gap-1"><Users size="12" /> {{ assign.submission_count || 0 }} submitted</span>
  ...
  <div @click="viewAssignmentDetail(assign)" class="flex items-center justify-between pt-4 border-t border-gray-100 text-sm text-gray-500 font-medium cursor-pointer hover:bg-gray-50 transition-colors">
    ...
    <el-dialog 
  v-model="showCreateCourseModal" 
  :title="isEditMode ? 'Update Course Info' : 'Add New Teaching Course'" 
  width="500px" 
  rounded-2xl
  @closed="handleCloseCourseModal"
>
    ...
    <el-dialog v-model="showManualKPModal" :title="editingKP ? 'Edit Knowledge Point (L2)' : 'Create Knowledge Point (L2)'" width="400px" rounded-2xl @closed="resetKPForm">
      <div class="space-y-4 p-2 text-left">
          ...
          <div>
              <label class="text-[10px] font-bold text-gray-400 uppercase ml-1">Bloom Level</label>
              <el-select v-model="kpManualForm.bloom_level" class="w-full">
          ...
          <div class="flex justify-end gap-2 pt-2">
              ...
              <el-button type="primary" color="#2563eb" @click="handleKPFormSubmit" :loading="isManualKPSaving">
                  {{ editingKP ? 'Update' : 'Save & Select' }}
              </el-button>
          </div>
      </div>
    </el-dialog>
...
import { 
  ...
  Sparkles, Zap, Edit, ChevronRight, PlusCircle, Copy, Trash2, LayoutGrid, List, Pencil
...
const kpManualForm = reactive({ name: '', description: '', bloom_level: 'apply' });
...
const isGeneratingRubric = ref(false);
...
const selectedKPs = computed(() => {
    return kpOptions.value.filter(o => form.knowledge_points.includes(o.id));
});
...
const handleCloseCourseModal = () => {
    ...
    courseCreateForm.name = '';
    courseCreateForm.description = '';
};
...
const handleEditCourseInfo = (course) => {
    courseCreateForm.name = course.name;
    courseCreateForm.description = course.description;
    editingId.value = course.id; // 复用这个 ID 变量
    isEditMode.value = true; // 复用这个模式变量
...
const handleCreateCourse = async () => {
    ...
    try {
        if (isEditMode.value && editingId.value) {
            // 更新逻辑
            await api.put(`/api/auth/teacher/courses/${editingId.value}/`, {
...
const confirmDeleteCourse = async (course) => {
    try {
        await ElMessageBox.confirm(
            `Are you sure to delete "${course.name}"? This action cannot be undone.`,
            'Critical Warning',
            { confirmButtonText: 'Delete', cancelButtonText: 'Cancel', type: 'error' }
        );
        const res = await api.delete(`/api/auth/teacher/courses/${course.id}/`);
        ElMessage.success(res.message || 'Course deleted');
        fetchCourses();
    } catch (e) {
        if (e !== 'cancel') {
...
// 🚀 删除作业逻辑
const handleDeleteAssignment = async (assign) => {
    try {
        // 1. 弹出二次确认框
        await ElMessageBox.confirm(
            `Are you sure you want to delete assignment "${assign.title}"? This action cannot be undone.`,
            'Critical Warning',
            {
        ...
        // 2. 发送 DELETE 请求
        const res = await api.delete(`/api/auth/teacher/assignments/${assign.id}/`);
        
        // 3. 成功提示
        ElMessage.success(res.message || 'Assignment deleted successfully');
        ...
        // 5. 捕获后端返回的错误（例如：已有提交无法删除）
        const errorMsg = e.response?.data?.error || 'Failed to delete assignment';
...
// 🚀 手动添加 KP
// 打开编辑弹窗
const startEditKP = (kpId) => {
    const kp = kpOptions.value.find(o => o.id === kpId);
    if (!kp) return;
    editingKP.value = kp;
    kpManualForm.name = kp.name;
    kpManualForm.description = kp.description;
    kpManualForm.bloom_level = kp.bloom_level || 'apply';
    showManualKPModal.value = true;
};

// 从选中列表移除 KP
const removeKP = (kpId) => {
    form.knowledge_points = form.knowledge_points.filter(id => id !== kpId);
};
...
const resetKPForm = () => {
    ...
    kpManualForm.name = '';
    kpManualForm.description = '';
    kpManualForm.bloom_level = 'apply';
};

// 统一处理创建和编辑
const handleKPFormSubmit = async () => {
    if (!kpManualForm.name || !kpManualForm.description) return ElMessage.warning('Incomplete form');
    isManualKPSaving.value = true;
    try {
        if (editingKP.value) {
            // 编辑模式
            const res = await api.patch(`/api/auth/knowledge-points/${editingKP.value.id}/`, {
                name: kpManualForm.name,
                description: kpManualForm.description,
                bloom_level: kpManualForm.bloom_level
            });
            const idx = kpOptions.value.findIndex(o => o.id === editingKP.value.id);
            if (idx !== -1) kpOptions.value[idx] = { ...kpOptions.value[idx], ...res };
            ElMessage.success('KP Updated');
        } else {
            // 创建模式
...
const generateAIRubric = async () => {
  // 安全拦截：确保第一步的内容已填写
  if (!form.content) {
    return ElMessage.warning('Please provide assignment requirements in Step 1 first!');
  }
  
  // 如果当前已经有填写的 Rubric，提示老师是否覆盖
  const hasContent = (form.rubric_config?.items || []).some(item => item.criterion.trim() !== '');
  if (hasContent) {
    try {
      await ElMessageBox.confirm(
        'This will replace your current rubric with AI-generated content. Continue?',
        'Warning',
  ...
  isGeneratingRubric.value = true;
  try {
    // 调用后端新写的 suggest-rubric 接口
    const res = await api.post('/api/auth/teacher/assignments/suggest-rubric/', {
      title: form.title,
      content: form.content,
      language: form.category
    });

    if (res.rubric && Array.isArray(res.rubric.items)) {
      // 将 AI 生成的数据填充到响应式表单中
      form.rubric_config.items = res.rubric.items.map(item => ({
        criterion: item.criterion,
        weight: item.weight || 0,
        description: item.description || '',
        detailed_rubric: item.detailed_rubric || {
      ...
      ElMessage.success('AI Rubric generated successfully!');
    }
  } catch (e) {
    console.error(e);
    ElMessage.error(e.response?.data?.error || 'AI Rubric generation failed.');
  } finally {
    isGeneratingRubric.value = false;
  }
};
...
const fetchCourses = async (page = 1, append = false) => {
  ...
  try {
    const res = await api.get(`/api/auth/teacher/courses/?page=${page}`);
    ...
    totalPages.value = Math.ceil((res.count || data.length) / 9);
