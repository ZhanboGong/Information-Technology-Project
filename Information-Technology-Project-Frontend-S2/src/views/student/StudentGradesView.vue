    ...
    <div v-else class="space-y-10">
      <div v-for="(records, courseName) in groupedRecords" :key="courseName" class="space-y-4">
        ...
        <div class="flex items-center justify-between border-b border-gray-100 pb-2">
          ...
          <button 
            v-if="records.length > initialDisplayCount"
            @click="toggleCourseCollapse(courseName)"
            class="text-xs font-bold text-gray-400 hover:text-blue-600 transition-colors flex items-center gap-1"
          >
            {{ collapsedCourses[courseName] ? 'Show All' : 'Show Less' }}
            <ChevronDown size="14" :class="{'rotate-180': !collapsedCourses[courseName]}" class="transition-transform" />
          </button>
        </div>

        <div class="grid grid-cols-1 gap-4">
          <div 
            v-for="record in getDisplayRecords(records, courseName)" 
            :key="record.id"
...
// 获取每个课程要显示的记录（折叠/展开控制）
const getDisplayRecords = (records, courseName) => {
  if (collapsedCourses[courseName] || records.length <= initialDisplayCount) {
    return records.slice(0, initialDisplayCount);
  }
  return records;
};

const toggleCourseCollapse = (courseName) => {
  collapsedCourses[courseName] = !collapsedCourses[courseName];
};
