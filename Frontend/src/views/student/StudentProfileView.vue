<template>
  <div class="space-y-6 animate-fade-in max-w-6xl mx-auto pt-2 pb-20">
    ...
    <div class="frosted-card p-6 rounded-2xl border border-white/60 bg-gradient-to-r from-blue-50/50 to-purple-50/30 shadow-sm">
      <div class="flex items-center gap-5">
        <div class="relative">
          <div class="absolute -inset-1 bg-gradient-to-tr from-blue-600 to-purple-600 rounded-full opacity-20 blur-sm"></div>
          <div class="relative h-16 w-16 rounded-full bg-white border-2 border-white shadow-sm flex items-center justify-center text-2xl font-bold text-blue-600">
            {{ user?.first_name?.[0] || user?.username?.[0]?.toUpperCase() || 'S' }}
          </div>
        </div>
        <div>
          ...
          <p class="text-xs text-gray-500 mt-1 flex items-center gap-2 font-medium">
            <span class="px-2 py-0.5 bg-blue-100 text-blue-700 rounded-md uppercase tracking-wider text-[10px]">Student Account</span>
            <span class="text-gray-300">|</span>
            <span>ID: {{ user?.student_id_num || 'N/A' }}</span>
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
      <div class="space-y-6">
        <div class="frosted-card p-6 rounded-2xl border border-white/60 bg-white/40">
          <h3 class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-5">Account Status</h3>
          <div class="space-y-3">
            ...
            <div class="flex items-center justify-between p-3 rounded-xl bg-white/50 border border-white/20">
              <div class="flex items-center gap-3 text-gray-700">
                ...
                <span class="text-xs font-semibold">Identity Auth</span>
              </div>
              <CheckCircle2 size="16" class="text-emerald-500" />
            </div>
          </div>
      ...
      <div class="lg:col-span-2 space-y-6">
        <div class="frosted-card p-6 rounded-2xl border border-white/60 bg-white/40 shadow-sm relative overflow-hidden">
          <h3 class="text-lg font-bold text-gray-800 mb-6 flex items-center gap-3">
            <div class="p-2 bg-blue-100 text-blue-600 rounded-lg"><UserSquare size="18" /></div>
          ...
          <form @submit.prevent="handleUpdateInfo" class="space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div class="space-y-1.5">
                ...
                <div class="relative group">
                  ...
                  <input v-model="infoForm.first_name" type="text" 
                         class="w-full pl-11 pr-4 py-2.5 text-sm rounded-xl border border-gray-200 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all bg-white/80" />
                </div>
              </div>
              ...
              <div class="md:col-span-2 space-y-1.5">
                ...
                <p class="text-[10px] text-orange-500 flex items-center gap-1 mt-1 ml-1 font-medium">
                  <AlertCircle size="12" /> To change your email, please contact the administrator.
                </p>
              </div>
            </div>
            ...
            <div class="pt-5 border-t border-gray-100 flex justify-end">
              <button type="submit" :disabled="infoSubmitting" 
                      class="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-xs shadow-md transition-all flex items-center gap-2 active:scale-95 disabled:opacity-50">
...
import { 
  User, Loader2, Lock, Mail, CheckCircle2, 
  ShieldCheck, UserSquare, Save, KeyRound, RefreshCw, AlertCircle 
...
const handleUpdateInfo = async () => {
  ...
  try {
    ...
    ElMessage.error('Failed to update information');
  } finally {
